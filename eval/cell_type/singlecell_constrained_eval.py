"""
Single-cell + Constrained Evaluation Script

This variant tests:
- Each batch is decomposed into N independent single-cell tasks
- Each task shows only ONE cell's gene expression
- The FULL candidate list of N types is still provided
- Models must choose from N options without seeing other cells

This tests the importance of batch-level context for reasoning.

Usage:
    python singlecell_constrained_eval.py \
...
def calculate_metrics(predictions: List[str], ground_truths: List[str], decomposed_data: List[Dict]) -> Dict:
    ...
    metrics = {
        "task_variant": "singlecell_constrained",
...
def run_evaluation(
    ...
    """Run single-cell + constrained evaluation."""
    ...
    print(f"\n[INFO] Task Variant: Single-cell + Constrained")
        ...
        for idx, (qa_item, response) in enumerate(zip(all_singlecell_qas, batch_responses)):
            try:
                ...
                result_item = {
                    "index": idx,
                    "task_variant": "singlecell_constrained",
                    "original_batch_index": qa_item["original_batch_index"],
                    "cell_index_in_batch": qa_item["cell_index_in_batch"],
                    "question": qa_item["question"],
    ...
    print("\n" + "="*60)
    print("SINGLE-CELL + CONSTRAINED EVALUATION")
    print("="*60)
    ...
    results_file = os.path.join(output_dir, f"singlecell_constrained_predictions_{timestamp}.json")
    with open(results_file, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    print(f"\n[INFO] Predictions saved to: {results_file}")

    metrics_file = os.path.join(output_dir, f"singlecell_constrained_metrics_{timestamp}.json")
...
def main():
    parser = argparse.ArgumentParser(description="Single-cell + Constrained evaluation")
