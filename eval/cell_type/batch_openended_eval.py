"""
Batch + Open-ended Evaluation Script

This variant tests:
- Models receive the full batch of N cells (batch-level context)
- NO candidate list is provided (open-ended generation)
- Models must freely generate cell type names for each cell

This tests whether batch-level reasoning helps even without candidate constraints.

Usage:
    python batch_openended_eval.py \
...
def run_evaluation(
    ...
    """Run batch + open-ended evaluation."""
    ...
    system_msg = (
        ...
        "Ensure that you consider all cells together, rather than annotating each cell individually. "
    ...
    try:
        ...
        # Process results
        for idx, (qa_item, messages, response) in enumerate(zip(qa_data, all_messages, batch_responses)):
            try:
    ...
    print("\n" + "="*60)
    print("BATCH + OPEN-ENDED EVALUATION")
    print("="*60)
    ...
    results_file = os.path.join(output_dir, f"batch_openended_predictions_{timestamp}.json")
    with open(results_file, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    print(f"\n[INFO] Predictions saved to: {results_file}")

    metrics_file = os.path.join(output_dir, f"batch_openended_metrics_{timestamp}.json")
...
def main():
    parser = argparse.ArgumentParser(description="Batch + Open-ended evaluation")
